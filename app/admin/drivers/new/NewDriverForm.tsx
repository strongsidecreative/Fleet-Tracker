...
function SubmitButton() {
  ...
  return (
    ...
    >
      {pending ? "Sending Invite…" : "Add Driver"}
    </button>
  );
}

export default function NewDriverForm() {
  const [state, formAction] = useFormState(createDriver, { error: null });
  ...
  return (
    <form action={formAction} className="space-y-4 rounded-xl border border-steel/20 bg-white p-4">
      ...
      <p className="text-xs text-steel">
        This sends an invite email through Supabase. The driver clicks the link and sets their own password.
      </p>
      {state.error && (
